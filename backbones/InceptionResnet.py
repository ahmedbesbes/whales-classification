...
class InceptionResnetV1(nn.Module):
    ...
    def __init__(self, pretrained=None, classify=False, num_classes=None, embedding_dim=512, dropout_prob=0.6, device=None):
        super().__init__()
        ...
        self.last_linear = nn.Linear(1792, self.embedding_dim, bias=False)
        self.last_bn = nn.BatchNorm1d(
            self.embedding_dim, eps=0.001, momentum=0.1, affine=True)
        self.logits = nn.Linear(self.embedding_dim, tmp_classes)

        if pretrained is not None:
            load_weights(self, pretrained)

        if self.num_classes is not None:
            self.logits = nn.Linear(self.embedding_dim, self.num_classes)
